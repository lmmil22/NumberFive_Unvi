...
//by수경 휴학/복학 승인대기/승인완료 클릭 시 라디오 값 변경(단일 변경)
function changeStatus(applyNo, processStatus){
...
function takeOffAllAccept(){
	...
	takeOffForm.querySelector('#takeOffInput').value = applyNos;
...
function comebackAllAccept(){
	...
	//applyNo를 담을 applyNo를 input 히든으로 데이터 담아간다.
	comebackForm.querySelector('#comebackInput').value = applyNos;
