...
public class memImgUploadFileUtil {
	// 내(유빈)컴퓨터 버전 경로
...
//------------------------------------------------------------------------------------------------------------------//
	...
	public static ImgVO uploadFile(MultipartFile memImage) {
		...
		if (!memImage.isEmpty()) {
			//첨부하려는 원본 파일명
			 originalFilename = memImage.getOriginalFilename();
			//파일이 첨부될 경로
